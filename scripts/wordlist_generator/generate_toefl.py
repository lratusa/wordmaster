#!/usr/bin/env python3
"""
TOEFL Word List Generator

This script generates a comprehensive TOEFL word list by:
...
Usage:
    ...
    # Then run:
    python generate_toefl.py

    # To resume from checkpoint:
    python generate_toefl.py --resume

    # To use only source data (no API calls):
    python generate_toefl.py --no-api

    # For dry run (test with 10 words):
    python generate_toefl.py --dry-run
"""
...
# Configuration
DEFAULT_SOURCE_PATH = "D:/temp/english-vocabulary/json/6-托福-顺序.json"
DEFAULT_OUTPUT_PATH = Path(__file__).parent.parent.parent / "assets" / "wordlists" / "english" / "toefl.json"
PROGRESS_FILE = Path(__file__).parent / "toefl_progress.jsonl"
...
def load_source_data(source_path: str) -> list[dict]:
    ...
    Args:
        source_path: Path to the TOEFL JSON file
...
def assign_difficulty(word: str, pos: str) -> int:
    ...
    TOEFL words are generally harder than CET-4, so we adjust the baseline.
    ...
    # Level 1: Common, short words (fewer in TOEFL)
    if len(word) <= 4:
        return 1
    ...
    # Check for complex suffixes (common in academic vocabulary)
    complex_suffixes = ['tion', 'sion', 'ment', 'ness', 'ical', 'ious', 'eous', 'ology', 'ative', 'itive']
    for suffix in complex_suffixes:
        if word_lower.endswith(suffix):
            return 3

    # Check for academic prefixes
    academic_prefixes = ['anti', 'auto', 'bio', 'geo', 'hyper', 'inter', 'micro', 'mono', 'multi', 'neo', 'pseudo', 'semi', 'sub', 'trans']
...
def generate_wordlist(
    ...
    output = {
        'name': 'TOEFL 托福词汇',
        'language': 'en',
        'description': f'托福考试核心词汇 (约{len(words_output)}词)',
        'icon_name': 'school',
        'words': words_output
    }
...
def main():
    parser = argparse.ArgumentParser(description='Generate TOEFL word list')
    parser.add_argument(
        '--source', '-s',
    ...
    parser.add_argument(
        '--resume',
        action='store_true',
        help='Resume from checkpoint (default behavior, kept for compatibility)'
    )
